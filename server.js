...
app.get('/', (req, res) => {
    if (process.env.BLOCK_TRAFFIC != undefined) {
      ...
      console.error('server-warn: An client attempting to connect to the API but it been blocked due to BLOCK_TRAFFIC is defined.');
      console.error('server-error: Client has the following headers:')
      console.error(req)
      res.status(418).json({okay: false, error: "Traffic is currently blocked by the server itself.", code: 418});
    } else {
...
app.get('/api/versions', (req, res) => {
    res.status(200).json({ok: true, data: { latest: "latestVersion", availableBuilds: "availableBuilds" } })
})
...
app.get('/api/debug', (req, res) => {
    if (process.env.NODE_ENV == "development") {
     ...
     res.status(501).send({ok: false, error: 'HTTP Request Debugger is disabled on this instance.', code: 501})
    }
})
...
app.use(function (req, res, next) {
  res.status(404).json({ok: false, error: "The endpoint you attempting to access is either not found or the URL path is wrong or you're attempting to do an POST in an GET-only endpoint.", code: 404})
})
